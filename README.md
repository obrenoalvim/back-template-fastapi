...
- `app/api/deps.py`'s `get_current_user` is the single dependency every protected route uses. It decodes and validates the bearer token, no per-route duplication.
...
`code` is one of `VALIDATION_ERROR`, `UNAUTHORIZED`, `FORBIDDEN`, `NOT_FOUND`, `CONFLICT`, `RATE_LIMITED`, `INTERNAL_ERROR`. It deliberately matches the shape used by `back-template-spring`/`back-template-nest`/`back-template-laravel`, so a front-end template can swap backends with minimal changes to its error-handling code.
...
1. **build**: `uv sync`, `ruff check`, `ruff format --check`, `mypy`, `alembic upgrade head`, `pytest`, all run against a real Postgres service container
